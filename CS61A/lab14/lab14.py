...
def prune_min(t):
    ...
    "*** YOUR CODE HERE ***"
    # if not t.is_leaf():
    #     t.map(lambda x: min(x.branches))

    if t.is_leaf():
        return
    else:
        # min_index = t.branches.index(min([l.label for l in t.branches]))
        # t.branches.pop(min_index)
        if t.branches[0].label < t.branches[1].label:
            t.branches.pop(1)
        else:
            t.branches.pop(0)
        # print("DEBUG:", t.branches)
...
def num_splits(s, d):
    ...
    # def partitions(lst):
        ...
        #         y = [x].extend(partitions(lst[i:]))
        #         part_sums.append(y)
        #     return part_sums
    # def parts(s, i):
    #     if len(s) == 0:
    #         return 1 if abs(i) <= d else 0
    #     first, rest = s[0], s[1:]
    #     return parts(rest, i + first) + parts(rest, i - first)
    # return parts(s, 0) // 2

    print("DEBUG:", partitions(s))
