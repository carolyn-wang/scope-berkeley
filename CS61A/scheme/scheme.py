...
def scheme_eval(expr, env, _=None): # Optional third argument is ignored
    ...
    if scheme_symbolp(first) and first in SPECIAL_FORMS:
        ...
        combined = scheme_apply(operator, args, env)
...
def do_define_form(expressions, env):
    ...
    if scheme_symbolp(target):
        ...
        # BEGIN PROBLEM 5
        symbol, value = expressions.first, expressions.rest
...
def make_let_frame(bindings, env):
    ...
    while bindings:
        subexpress = bindings.first
        validate_form(subexpress, 2, 2)

        formals = subexpress.first
        elements = subexpress.rest.first

        names = Pair(formals, names)
        values = Pair(scheme_eval(elements, env), values)
