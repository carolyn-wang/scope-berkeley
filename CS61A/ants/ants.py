...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self, beehive):
        ...
        while current_place:
            ...
            elif current_place.bees and self.min_range <= i <= self.max_range:
                return rANTdom_else_none(current_place.bees)
            
            else:
                current_place = current_place.entrance #update current place to min
                i+=1
        # END Problem 3 and 4
...
# END Problem EC
    ...
    def action(self, gamestate):
        ...
        else: #true queen actions
            super().action(gamestate)
            position = self.place
            while position.exit != None:
                position = position.exit
                if position.ant and not position.ant.doubled:
                    position.ant.damage *= 2
                    position.ant.doubled = True
